...
const momentWithRange = MomentRange.extendMoment(moment);
...
export const abSort = (a, b) => (b.y - a.y);
...
export const markerToString = (marker) => `${marker.lat || ''},${marker.lng || ''}`;
...
export const stringWrapper = (str, width) => {
...
export const removeAsset = asset => {
  if (!asset) return '-';
  return asset?.replace('Asset', '');
};

export const getDirectDataFirebase = async (url) => (axios.get(url));

export const dateRange = (startDate, endDate) => Array.from(momentWithRange().range(startDate, endDate).by('days')).map(d => d.format('YYYY/M/D'));
